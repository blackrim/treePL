...
struct ADmemblock {	// We get memory in ADmemblock chunks
...
class ADcontext {	// A singleton class: one instance in radops.c
...
class ADvari {	// implementation of an ADvar
public:
...
class ADvar {		// an "active" variable
    ADvari *cv;
public:
    ADvar() { /*cv = 0;*/ }
    ADvar(double);
...
class ADvar1: public ADvari {	// simplest unary ops
public:
    Derp d;
...
class ADvar1s: public ADvar1 { // unary ops with partial "a"
public:
    double a;
...
class ADvar2: public ADvari {	// basic binary ops
public:
    Derp dL, dR;
...
class ADvar2q: public ADvar2 { // binary ops with partials "a", "b"
public:
    double a, b;
